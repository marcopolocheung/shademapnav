 ...
 *
 * A4 swaps routing's shadow source from the map canvas to `ShadowField`. Before that
 ...
 * - **the blue-dominant predicate round-trip** — whether shadow painted in the
 *   renderer's colours is still recoverable as shadow (invariant #5, which nothing
...
export function fieldAnswerFor(fixture: AgreementFixture): { left: number; right: number } {
  ...
  const [shadow] = createGeometryShadowField([provider]).sampleEdges([fixture.edge], fixture.when);
  return { left: shadow.left, right: shadow.right };
}
...
export function formatReport(report: AgreementReport): string {
  ...
  return [
    `shadow-field agreement: ${report.cases} cases`,

 ...
 * and `offscreenShadow.ts`, and the whole point of that move was that nothing about
 ...
 *
 * Delete it once `ShadowField` (A2) owns the geometry contract outright and the
...
describe("A1 extraction parity", () => {
  ...
  it("answers point queries identically across a sampling grid", () => {
    ...
    expect(shadowedCount).toBeGreaterThan(50);
  });
});

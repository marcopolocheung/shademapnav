...
describe("dose — the full-sun equivalent shown to users", () => {
  ...
  it("counts shaded minutes at the transmission band, not at zero", () => {
    const d = dose({ sunMinutes: 0, shadeMinutes: 60 }, 8)!;

    expect(d.fullSunEquivalentMinutes.low).toBeCloseTo(60 * SHADE_UV_TRANSMISSION.low, 6);
    expect(d.fullSunEquivalentMinutes.high).toBeCloseTo(60 * SHADE_UV_TRANSMISSION.high, 6);
  });

  it("is independent of the UV index — it is a duration, not a dose", () => {
    const trip = { sunMinutes: 20, shadeMinutes: 20 };
...
describe("dose — shade is not a shield", () => {
  it("charges shaded minutes a real share of the ambient rate", () => {
    ...
    // 60 min at UV 8 in full sun would be 7.2 SED; in shade, 20–60% of that.
...
describe("dose — honesty about what it does not know", () => {
  ...
  it("reports every figure as an interval, never a point value", () => {
    const d = dose({ sunMinutes: 20, shadeMinutes: 20 }, 8, { skinType: "II" })!;
  ...
  it("reports the transmission band the documented derivation uses", () => {
    ...
    expect(SHADE_UV_TRANSMISSION).toEqual({ low: 0.2, high: 0.6 });
  });

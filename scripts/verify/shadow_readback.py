#!/usr/bin/env python3
"""
Browser verification for the flat shadow readback (#154).

`npm test` runs in `environment: "node"` and has never executed a browser, so the
one claim that matters here cannot be checked by the four gates: that route shadow
...
shadow percentages the route cards report. Before this fix they diverge, because the
shadow layer paints buildings with the same blue field the sampler reads as shadow.
...
# Pick a time when shadow is *partial*. Late afternoon in Midtown saturates every
...
CALCULATE = "Find Shadowed Route"

SHADOW_RE = re.compile(r"^\d+% shadow$")


def shadow_labels(page) -> list[str]:
    """Every distinct '42% shadow' the route cards are showing."""
    texts = page.eval_on_selector_all(
        "*", "els => els.map(e => (e.textContent || '').trim())"
    )
    return sorted({t for t in texts if SHADOW_RE.match(t)})
...
def run_case(page, url: str, tilt: bool, out: Path, tag: str) -> dict:
    ...
    return {
        ...
        "shadowLabels": shadow_labels(page),
    }
...
def main() -> int:
    ...
    (args.out / "shadow_readback.json").write_text(json.dumps(report, indent=2))
    ...
    if flat["shadowLabels"] != tilt["shadowLabels"]:
        failures.append(
            f"shadow depends on pitch: flat {flat['shadowLabels']} "
            f"vs tilted {tilt['shadowLabels']}"
        )
    if not flat["shadowLabels"]:
        failures.append("no route cards rendered — the scene or selectors are stale")
    if flat["shadowLabels"] == ["100% shadow"]:
        failures.append(
            "shadow is saturated at 100%, so the flat/tilted comparison proves "
            "nothing — pick a --time with partial shadow"
        )
    ...
    if failures:
        return 1
    print("PASS: shadow is identical flat and tilted, and the tilt came back.")
    return 0

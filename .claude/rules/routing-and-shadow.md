...
# Routing, the cost model, and shadow sampling
...
Behavior changes to `routing.ts`, `trainGraph.ts` and `shadowSampling.ts` require test
...
Shadow is one term in a multi-objective cost, not a filter. The product's value is the
*tradeoff* — a route 6% longer and 40% more shadow — so keep the Pareto structure intact rather
...
`isBlueDominantShadowPixel` is the shared predicate that defines "shadowed" for the whole app,
...
makes "cross to the shadowed side" possible. Do not average it away for a tidier return type.

## Honesty

`shadowCoverage: 0..1` is a blue-pixel fraction. It is not a measurement, and the agreement

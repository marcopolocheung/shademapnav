# Umbra: a distinctive product and a credible engineering portfolio

**Recruiter-focused innovation and implementation roadmap**  
**Prepared for:** Marco Cheung  
**Research date:** September 7, 2026  
**Scope:** Current first-party SWE and AI/ML hiring descriptions, selected source-level inspection of Umbra, and a proposed product/technical roadmap.
...
I would position Umbra around this promise:
...
An illustrative future request is: "I have 90 minutes. Find coffee and somewhere shadowed to read for 20 minutes, keep my estimated direct-sun exposure below eight minutes, and get me back before 5:30."
...
My preferred combination is **Sun Budget + Living Itinerary + ShadowBench**. It creates one coherent product and gives you algorithmic, full-stack, applied-AI, testing, and operational stories. For a stronger ML-engineering emphasis, add a narrow **Reality Check** model trained and evaluated on independently observed shadow. For a systems emphasis, add **City Capsules**, which make a neighborhood usable offline.
...
| The Pareto formulation includes shadowed distance as a maximized quantity. [Source](https://raw.githubusercontent.com/marcopolocheung/shademapnav/main/app/lib/routing.ts) | Accumulating more shadowed meters can also accumulate more exposed meters. | Make exposure duration and explicit user constraints the main product objective. |
| `plan_shadowed_route` starts a calculation and returns a "started" response; it does not await the resulting route. [Source](https://raw.githubusercontent.com/marcopolocheung/shademapnav/main/app/lib/agent/tools.ts) | Tool-call success is not plan success. | Return a job/route identifier and completed or failed result with metrics and provenance. |
...
The arithmetic behind the objective concern is simple. Route A has 1,000 meters total and 800 shadowed: 200 exposed. Route B has 1,500 total and 1,000 shadowed: 500 exposed. B contains more shadowed distance but also more exposure. This is a constructed example, not a measured failure of your deployed app.

## 4. Flagship feature: Sun Budget

### The user experience

Move from "give me a route with more shadow" to:
...
estimated exposure seconds =
    sum(segment duration * unshadowed fraction during traversal)
...
Start with a single neighborhood graph. Retain your TypeScript implementation and run expensive work away from the UI thread. Use a small time-expanded reference model or enumeration on toy graphs as the correctness oracle. For the interactive implementation, investigate bounded label search and time-bucketed shadow queries.
...
Use one reproducible fixture where the currently most shadowed path becomes less attractive by the time the walker reaches it. Compare the existing static method with the new method. Publish constraint violations, approximation gap on small exact fixtures, runtime distribution, memory, and cache behavior.
...
Do not call a probabilistic learned shadow value "verified truth." Verification means consistency with explicit constraints and available evidence; physical accuracy remains a separate evaluation.
...
### ShadowBench: the part that makes the AI work credible
...
> "The map says this sidewalk should be shadowed. Reality disagrees. Help it learn."

A user can flag a missing tree canopy, awning, changed building, or incorrectly predicted shadow. The app identifies uncertain areas and can explain why a recommendation is less confident.
...
**Simple learned correction:** logistic regression or gradient-boosted trees predicting observed shadow or geometry-error risk from geometry confidence, solar position, street orientation, available canopy features, and observation conditions.
...
Measure false-shadow predictions, observation-level accuracy, exposure-time error over routes, and changes in route decisions. Show results by coverage, geometry source, canopy presence, solar angle, and location type.
...
Keep the existing React/MapLibre interface. Put the routing/shadow computation behind a worker boundary. Use browser storage for local snapshots and an atomic manifest swap so a half-downloaded update cannot replace a working capsule.
...
Extend preferences from avoiding sun to seeking it at selected stops: sunny breakfast, shadowed reading, sunset viewpoint. The advantage is one time-space planning model serving different preferences, rather than an unrelated recommendation engine for each theme.
...
Account for partition boundaries: a tall building outside a tile can cast shadow into it. Choose a justified halo or query extent and test it at low solar angles. Define polar/night/low-sun behavior and resource limits rather than allowing unbounded extents.
...
|---|---|---|---|
| Geometry | Synthetic analytic fixtures and independent sampled observations | Intersection errors, false shadow, uncertainty by source | Testing a renderer against the same geometry and calling agreement physical accuracy. |
...
| ML | Held-out geographic and temporal observations | Prediction error, reliability, false shadow, decision impact | Random-frame leakage and tuning on the test set. |
...
- "Trained and calibrated a geospatial shadow-correction model with neighborhood/date holdouts; measured [prediction metric] and [route-level impact], with versioned deployment and rollback."
...
**Milestone 2 - Living Itinerary and ShadowBench.** Introduce typed plan/jobs, completed-result tools, plan revisions, validation, cancellation, and a first held-out benchmark. Make one impressive repair scenario work end to end.
...
> **Sun Budget + Living Itinerary + a public evaluation suite, followed by a small real-world shadow-learning pilot.**
...
Repository inspection was selective, source-level, and not pinned to a single commit. Documentation and code had some disagreements; conclusions prioritize the inspected implementation and acknowledge that limitation. I did not execute tests, verify production behavior, measure traffic, assess real-world shadow accuracy, or complete a comprehensive security review. Source-access gaps were not filled with invented implementation details.

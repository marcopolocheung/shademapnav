...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0.5 km box | 276x276 | 1.82 | 2 | 41.2 KB | 60.4 KB | 60.4 KB | 1.47x | 26 | <0.1% |
| 2 km box | 1101x1101 | 1.82 | 9 | 259.7 KB | 279.0 KB | 279.0 KB | 1.07x | 47 | 0.3% |
| 5 km box | 2751x2751 | 1.82 | 42 | 1430.5 KB | 1450.0 KB | 1450.0 KB | 1.01x | 146 | 1.8% |
...
- **requested / transferred** — the byte ranges `geotiff` asked for, against what
  ...
  cloning bodies undercounts once Chromium's cache is involved.
...
tile counts. **Requested equals transferred on every read**, which is worth stating
plainly: ranges are honoured exactly, with no over-delivery and no silent fall back
to serving the whole 84 MB object.

**Overhead is a fixed cost, not a rate.** The ~19 KB gap between payload and
transferred is the IFD chain walk, and it is the same ~19 KB at every AOI size — so
it is 47% on a 0.5 km box and 1% on a 5 km one. **A8b should cache the opened
`GeoTIFF`, not just decoded tiles**: reopening a tile costs that walk again, plus
seven of the read's round trips.

## Time, round trips and memory

| AOI | open | read + decode | total | round trips | raster | heap held over baseline |
|---|---:|---:|---:|---:|---:|---:|
| 0.5 km box | 5348.8 ms | 1584.2 ms | 7155.3 ms | 26 | 74.4 KB | 2242.0 KB |
| 2 km box | 3057.1 ms | 4805.4 ms | 8085.2 ms | 47 | 1183.8 KB | 7369.8 KB |
| 5 km box | 2997.2 ms | 17755.2 ms | 20986.4 ms | 146 | 7390.6 KB | 9956.4 KB |

**The cost is round trips, not bytes.** 1.4 MB is nothing; 146 sequential range
requests at 1.5-3.5 s each is everything. `read + decode` tracks the request count
far better than it tracks the byte count, and `open` is roughly constant because
the IFD walk is a fixed seven-ish requests wherever it happens. Any A8b work that
reduces latency should reduce **round trips** — `geotiff`'s `maxRanges` for
multi-range requests, a larger `blockSize`, or fetching tiles concurrently — before
it tries to reduce bytes. Do not read the milliseconds as a device budget; they are
a domestic connection to a research mirror, and they varied by 5x across runs
while every byte column was identical on all five.

**Memory is roughly the raster plus a fixed ~2 MB.** The figure is what the reader
*retains* with the raster live, against a `HeapProfiler.collectGarbage` baseline —
not the transient peak inside `readRasters`, which nothing available in a browser
can see. Two instruments were discarded getting there, both worth not repeating:
polling `performance.memory` inflated decode **5-7x** (with
`--enable-precise-memory-info` every read does real work, so the instrument landed
inside its own measurement — it is now read exactly twice), and CDP's
`Runtime.getHeapUsage` counts only the V8 heap, so `ArrayBuffer` backing stores are
invisible to it and the 7.4 MB raster reported as **1.0 MB**, *falling* as the AOI
grew.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Madrid | 8.27 MB | 4 | 198,332 | 2 | 1,966 B | **4208x** | 100.0% | 0.0% | 0.20% |
| Singapore | 24.39 MB | 30 | 586,086 | 26 | 6,114 B | **3990x** | 100.0% | 0.0% | 1.90% |
| Kent, WA | 4.72 MB | 4 | 112,143 | 2 | 1,357 B | **3476x** | 100.0% | 0.0% | 0.17% |

**37.4 MB of GeoJSON becomes a 9.8 KB committed file**, and the browser fetches
none of it. *Disagreement* is against the same polygons rasterized 4x finer — the
error is a band one cell wide along each imagery seam, which is why Singapore's 26
dates and 98 rings cost ten times Madrid's two and six. Reproduce the table with
`node scripts/canopy-acq-index.mjs --report`.

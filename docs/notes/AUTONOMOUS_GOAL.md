...
| `GROWTH_ROADMAP.md` | The *user/product thesis* — who the users are, why shadow matters. Written 2026-07-05. Still true; several of its items have since shipped. |
...
shadow-and-heat navigation landscape. It replaces "pick the highest-priority open issue"
...
> **A fast, accessible, shadow-first navigation webapp for people moving under their own
> power — walking, biking, scootering, skateboarding, running, wheeling.**

Three words decide every judgment call, in this order:

1. **Trustworthy** — never claim shadow the app can't deliver, never narrate a trip it
...
3. **Accessible** — both senses: real a11y (keyboard, screen reader, contrast, touch
   ...
   and doesn't know that dark blue means shadow.
...
- Shadow-aware pedestrian routing with Pareto options (shortest / balanced / most-shadowed),
  per-sidewalk shadow sampling, detour-budget pruning (`app/lib/routing.ts`, 1173 lines).
...
- Umbra Assistant — a real tool-using agent loop over Cerebras (`app/lib/agent/`, 8 tools:
  `locate_user`, `geocode_place`, `search_places`, `check_shadow`, `set_time`, `plot_points`,
  `plan_shadowed_route`, plus pre-injected `get_current_context`).
- Routing instrumentation with three KPIs at `window.__umbraMetrics` (`app/lib/metrics.ts`).
...
- ~~**`app/lib/bestTime.ts` is orphaned.**~~ **Fixed by D1.** `useHourlyExposure.ts` imports it
  and `HourlyExposureStrip` renders the day's shadow for the selected route. Still open against
...
- ~~**Neither canvas-free probe is generalized into a real shadow field.**~~ **Fixed by A1–A4b.**
  `app/lib/shadowField/ShadowField.ts` is the field, with `source` + `confidence` per sample, a
  prebuilt shadow index (#166), and `shadowProvenance.ts` aggregating over the path. Note A4b's
...
2. ~~**Shadow is read off the visible canvas.**~~ **Largely fixed by A4b** — `useNavigation.ts`
   samples `ShadowField` and falls back to pixels per edge. Worker offload (#38) is still open
   (**A5**), and the assistant no longer hijacks the camera for shadow probes (`tools.ts` uses
   `queryPointShadow`/`queryOffscreenBuildingShadow`). **What is still true, and is the live
   defect:** every edge is sampled at one `dateRef.current` (`:633`, `:1154`), so a long trip
   is priced as an instant — that is **Track H's H1**.
3. **Buildings are the only shadow source.** No trees (#46), no awnings, no arcades, no
   terrain. On the tree-lined streets where shadow-seekers actually walk, the app under-reports.
4. **Shadow is binary and unitless.** `shadowCoverage` is a 0–1 fraction. Users experience heat,
...
|---|---|---|
| **Google Maps "Prefer shade"** | A shadow toggle for walking directions surfaced in APK teardowns (Nov 2025), reportedly using Street View lidar/light data, plus an estimate of time in direct sun. | The *toggle* is being commoditized. A shadow checkbox will not be a differentiator in 12 months. Depth (time dimension, heat units, planning) will be. |
| **Google Maps + Gemini "Ask Maps"** | Rolled out 2026-03-12: conversational queries over 250M places, hands-free assistance during navigation, immersive 3D guidance with landmark-based voice cues. | Conversational map Q&A is now table stakes, and users' bar for it is Gemini's. Our assistant must be *grounded and honest* rather than broad — it wins by answering questions Google can't (shadow at 5:40pm on this block). |
...
| **CoolPath / CoolPaths (2026 papers)** | UTCI at 2 m grid, street-scale PET mapping from open data, cooler-route planning. | Confirms the direction: heat index, not shadow fraction. Also confirms open data is sufficient — no proprietary lidar required. |
| **Shadehopper**, **Geuneullo** (KR) | Consumer shadow-routing apps; Geuneullo models building *and street-tree* shadows. | Consumer competitors already model trees. Our buildings-only shadow is behind the consumer state of the art, not just the research one. |
...
| **Meta + WRI canopy height** | Global 1 m tree canopy height, updated 2026, MAE 2.8 m, free on AWS/GEE. | Global tree shadow is now *possible without a survey*. Overpass `natural=tree` is the cheap v1; this raster is the credible v2. |
| **Overture Buildings** | 780M+ footprints with height/levels, merged from OSM + AI footprints. | A fallback for cities where OSM building heights are missing — the main cause of "the app says sun and it's shadow". |
...
**The conclusion that drives §3:** shadow-as-a-toggle is being absorbed by the incumbents.
...
an *assistant that is grounded in the shadow model itself*. Everything below is chosen against
that thesis.
...
### Track A — Shadow Engine: a headless, time-sweepable, canopy-aware shadow field

**Problem.** Shadow is currently a screenshot. `useNavigation` renders the map, reads back
pixels, and calls blue "shadow". That single decision caps five other tracks: routing can't
move to a worker (#38), a time sweep costs a re-render per hour, the assistant must fly the
camera to probe a location, routes longer than the viewport degrade, and no shadow source
other than "what the renderer drew" can ever be counted.

**The bet.** Extract a pure, viewport-independent **shadow field**: given a coordinate (or an
edge) and a time, return a shadow fraction with a provenance and a confidence — computed from
...
- **A1 — Geometry extraction, decoupled.** Pull building-geometry assembly out of
  `LocalShadowAdapter` into `app/lib/shadowField/geometry.ts`: footprints + heights from vector
...
- **A2 — The `ShadowField` interface.** Define and implement
  `shadowAt(lng, lat, when): ShadowSample` and `sampleEdges(edges, when): Float32Array`, where
  `ShadowSample = { shadow: 0..1, source: "tiles" | "overpass" | "canopy" | "mixed", confidence: 0..1 }`.
...
- **A4 — Routing reads the field, not the canvas.** Swap `calculateRoute`'s shadow source
...
- **A5 — Worker offload.** Move graph build + shadow sampling + Dijkstra into a worker
...
- **A7 — Canopy v1 (Overpass).** `natural=tree`, `landuse=forest`, `leaf_type`, tree rows;
  ...
  *Done when:* a tree-lined street in Madrid/Barcelona reports materially more shadow, and
...
- **A9 — Beyond binary (stretch, feeds Track D).** Export not just "shadowed/not" but the
  ingredients of a radiant load: sky view factor per sample, sun altitude, surface class.

**Owns.** `app/lib/shadowField/**` (new), `app/lib/shadow/**`, `app/lib/shadowSampling.ts`,
`app/workers/**`, `app/lib/overpass.ts` (query additions).

**Must not.** Change shadow *colors* (invariant #5 couples them to the shadow predicate),
break `preserveDrawingBuffer` (#3), or touch UI beyond a debug overlay.

**Contract published to other tracks.** `ShadowField` (A2) + `sweep` (A6). Nobody else
implements shadow math. Ever.

---

### Track B — Live Navigation: turn-by-turn shadow guidance

**Problem.** The app is named for navigation and cannot navigate. `NAVIGATING` is a card
with a distance on it. Meanwhile Google is adding shadow *inside* real turn-by-turn with
landmark voice cues. A route you must hold in your head is a planning tool, not a navigator.

**The bet.** Guidance is where shadow routing becomes visceral: *"in 40 m, cross to the shadowed
side of Calle Mayor."* No competitor gives a shadow instruction, because none of them models
which **side of the street** is shadowed — and we already do (`sampleBothSidewalks` splits every
...
- **B4 — Guidance UI.** Replace the static `NAVIGATING` card: next maneuver, distance to it,
  remaining distance/ETA, a shadow strip for what's ahead, big touch targets, screen wake lock.
...
- **B5 — Off-route + reroute.** Detect departure beyond tolerance for N samples, recompute
  from current position, preserve the shadow preference. *Done when:* the simulated track can
  deviate and recover without a manual re-plan.
- **B6 — Shadow-aware cues.** Using Track A's per-sidewalk field: "cross now — the north side
  is shadowed for the next 300 m". *Done when:* cues fire only when the shadow delta exceeds a
  threshold (no chatter) and are suppressed when solar intensity or cloud cover makes them moot.
- **B7 — Voice (Web Speech API) + arrival summary.** "You walked 78% in shadow — about 4
...
**Must not.** Reimplement routing (call `useNavigation`'s pipeline) or shadow math (Track A).

**Contract published.** `Guidance` model: `{ maneuvers, activeIndex, progressM, offRoute, eta }`.

---

### Track C — Shadow Copilot: the assistant that is actually grounded in the map
...
guarantee" line injected into the write prompt), and `check_shadow` no longer moves the camera
(`tools.ts:359` → `queryPointShadow` → Overpass fallback). **Neither has ever been confirmed in
a browser (#59).** What remains genuinely open: two-point-only planning (`plan_shadowed_route`,
...
**The bet.** The Copilot's only job is to answer questions that require the shadow model:
"where can I sit outside in shadow at 4pm near here?", "plan a shadowed afternoon: coffee → park
...
- **C3 — Probes on the `ShadowField`.** `check_shadow` calls Track A's field (rather than the
  ...
  becomes a confident sentence, and "when is this terrace shadowed?" costs one tool call.
- **C4 — Multi-stop planning tool.** `plan_shadowed_route` accepts ordered stops and drives the
...
- **C5 — Answers with receipts.** Structured output: each claim carries the tool result that
  produced it; the UI renders chips ("Shadow 62% at 16:00 — checked") that focus the map object.
...
implement shadow/route logic outside a tool wrapper.

---

### Track D — Heat & Timing: shadow in human units
...
Heat units make the shadow tradeoff mean something to the sun-sensitive segment — the
...
- **D1 — Ship the orphan.** Wire `buildHourlyExposureSeries` into an exposure-by-hour chart
  for the *current route* ("70% shadowed before 10am, 25% at 2pm") with a tap-to-jump on each
...
- **D6 — Best-time surfaces.** "Best window today" on the home screen and on any saved route;
  a compact hour strip; cloud-aware ("shadow matters less at 14:00 — 90% cloud").
- **D7 — Morning routine (needs F PWA work).** Optional local notification: "today's most shadowed
...
- **E1 — Mode selector + real cost model.** Wire `TRAVEL_MODE_POLICIES` into `dijkstra`'s edge
  ...
  OSM). Closes #45.
- **E2 — Mode-aware output.** Speed, ETA, and the tradeoff sentence adapt per mode; shadow
...
- **E6 — Mixed-mode journeys.** Walk + transit is already half-built (`trainGraph.ts`);
  generalize to walk↔bike↔transit legs within one `Trip`, with per-leg shadow accounting
...
- **E8 — Saved journeys.** Home/Work + a commute `Trip` that reopens with today's shadow.
...
**Must not.** Redefine what "shadow" means (Track A) or own live position (Track B).
...
- **F3 — Share target + deep links.** PWA `share_target` and a documented URL scheme so an
  address shared from another app lands in a shadow route. Closes #68.
...
- **F5 — City landing pages.** A handful of pre-rendered pages ("shadowed walking routes in
...
- **F6 — Public API / embeddable widget (stretch).** Once Track A is a real engine, a
  read-only shadow-at-point endpoint is a link magnet for urbanists — the segment that
...
- **G2 — Route benchmark.** A repeatable scripted 2-point and 5-point calculation reporting
  `window.__umbraMetrics` numbers; commit the baseline. Unblocks #37/#38 (nobody may claim
  a perf win without it).
- **G3 — Bundle budget in CI.** Fail the build on regression beyond a stated ceiling. Closes #57.
- **G4 — Shadow accuracy harness.** Own the fixtures for Track A's A3 agreement number so the
...
`paretoRoutes` maximizes *shadowed distance* under a `2.0×` detour budget (`routing.ts:544`), so
"Most Shadowed" can carry more absolute exposed metres than "Shortest". `longestContinuousSunM`
is already computed and never enters the search.

**The bet.** This is the one thing on the board that nobody else ships. Google has a shadow
...
G1 G2 G3 (proving ground)  ─────────────────────────► every other track's "done"
        │
A1 → A2 (ShadowField) ─┬─► A3 → A4 → A5 (worker)
...
landed; `G2` (is there a committed baseline to claim against?), `A2` (`ShadowField`) — landed,
...
|---|---:|---|---|
| `app/hooks/useNavigation.ts` | 1445 | `useRouting` (pipeline), `useTrip` (waypoints/legs/saved), `useSketch` | E owns `useTrip`/`useSketch`; A owns the shadow-source seam inside `useRouting` |
...
- **Hard invariants in root `CLAUDE.md` are non-negotiable** (maplibre pinned at `5.9.0`;
  `preserveDrawingBuffer`; `MapView` only via `React.lazy`; shadow color ↔ shadow predicate
...
- **Honesty is a feature.** Any number the UI shows (shadow %, dose, ETA, heat score) must be
...
|---|---|---|
| A | Shadow-field vs pixel-sampler disagreement (pp); % of edges with `confidence > 0.8`; route calc p50 | G4 fixtures, `window.__umbraMetrics` |
...
- **Chasing Google's feature list.** When they ship "Prefer shadow", the answer is not a better

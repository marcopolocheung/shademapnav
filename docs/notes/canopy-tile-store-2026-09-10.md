...
It is one AOI in one city, so it does not generalise to Singapore or Kent on its own; the
mechanism to check them is now in place and costs a scan. `CanopyPatch.valid` is `null`
exactly when every pixel is valid, which is why an all-valid read allocates nothing for it.

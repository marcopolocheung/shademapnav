# The one-hour max-shadow window: declined, 2026-09-09

**Decision: declined for now.** Issue **#245** asked for one of two outcomes — adopt
Wen et al. 2025 §2.3.2's one-hour maximum-shadow window with its width, rationale and
...
Wen et al. smooth their shadow estimate over a one-hour window and take the maximum:
a point shadowed at 08:00 but sunlit at 09:00 counts as shadowed for the whole 08:00–09:00
slot. Their justification is behavioural — a pedestrian will step a few metres to stay
in shadow, so a sample landing just outside a shadow edge is not evidence the walker
will be in sun. Source: `docs/research/shadow-thermal-comfort-literature-2026-09-09.md`
§6, from `docs/research/shadowstudy.pdf`.
...
harness a comparison of shadow *models* rather than of sampling densities. A windowed
...
The direction matters. `ShadowField.ts` already refuses to call model-vs-model agreement
accuracy; adopting a change that reports **more** shadow, unmeasured, is exactly the
...
- **A reference that can score it.** A corpus of "I was standing here at this time and
  I was in sun / in shadow" observations — Wave 4 Option A's *cheap* calibration set —
...
- **A displayed-aggregate home for it.** #245's own strongest framing is that this is a
  ...
  never inside `ShadowField`, where H1 and the routing cost model would inherit it.
